...
class CrashReporter {
  ...
  ///
  ///   await CrashReporter.init(config);
  ///   runApp(const MyApp());
  /// }
  /// ```
  ...
  ///
  /// Call this from the AuthInterceptor whenever a response is received:
  /// ```dart
  /// final traceId = response.headers.value('X-Trace-Id');
  /// if (traceId != null) CrashReporter.setLastTraceId(traceId);
  /// ```
  ...
  ///
  /// Use for API errors, parsing failures, unexpected states.
  /// ```dart
  /// } catch (e, stack) {
  ///   CrashReporter.recordError(e, stack, reason: 'Failed to load profile');
  /// }
  /// ```
  ...
  ///
  /// ```dart
  /// CrashReporter.log('Navigated to /attendance');
  /// CrashReporter.log('Check-in tapped at lat=24.7, lng=46.7');
  /// ```
